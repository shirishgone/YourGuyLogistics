/*! Vendor-Webaap - v0.0.0 - 2016-04-12 */
!function() {
    "use strict";
    ...
    var a = function(a, b, c, d, e, f, g) {
        ...
        this.toggleFilter = function() {
            ...
            g.loaderStart();
            for (var a = "", b = Math.ceil(moment.duration(moment(h.params.end_date).diff(moment(h.params.start_date))).asDays()), c = {
                start_date: moment(h.params.start_date).startOf("day").toISOString(),
                end_date: moment(h.params.end_date).endOf("day").toISOString()
            }, d = 0; b > d; d++) a += ",IsoToDate(attendance ->" + d + "-> date) AS Date_" + (d + 1) + ",attendance ->" + d + "-> worked_hrs AS Hours_" + (d + 1);
            f.dgsAttendance.query(c, function(b) {
                var c = "SELECT name AS Name" + a;
                alasql(c + ' INTO XLSX("attendance.xlsx",{headers:true}) FROM ?', [ b.payload.data ]), 
                g.loaderComplete();
            });
    ...
    function a(a, b, c, d, e) {
        ...
        }, f.DG.shift_time = f.shift_timings[f.findShiftTime(f.DG.shift_time)], f.cancel = function() {
            a.cancel();
        }, f.answer = function(b) {
            a.hide(b);
        };
