/*! Vendor-Webaap - v0.0.0 - 2016-03-12 */
!function() {
    "use strict";
    ...
    var a = function(a, b, c, d, e, f, g, h) {
        ...
        var j = d.confirm().parent(angular.element(document.querySelector("#body"))).clickOutsideToClose(!1).title("Are you sure you want to Sign Out?").textContent("After this you will be redirected to login page.").ariaLabel("Sign Out").targetEvent().ok("Sign Out!").cancel("Not Now").openFrom("#logout-button").closeTo("#logout-button");
        this.toggleSideNav = function() {
            ...
            d.show(j).then(function() {
                i.logout();
            });
        }, a.$on("errorOccured", function() {
            h.loaderComplete(), a.errorMessage && e.show({
            ...
            h.loaderComplete(), a.successMessage && e.show({
    ...
    } ]).controller("homeCntrl", [ "$rootScope", "$state", "$mdSidenav", "$mdDialog", "$mdToast", "constants", "UserProfile", "Notification", a ]).controller("ErrorToastCntrl", [ "$mdToast", "$rootScope", function(a, b) {
    ...
    "use strict";
    angular.module("ygVendorApp", [ "ngMaterial", "ngMessages", "ui.router", "ngStorage", "ngResource", "base64", "login", "home", "order", "deliveryguy", "vendor", "reports", "forbidden" ]).config([ "$urlRouterProvider", "$locationProvider", "$resourceProvider", "$mdThemingProvider", "roleProvider", function(a, b, c, d, e) {
    ...
    "use strict";
    var a = function(a, b, c, d, e, f, g, h, i, j, k, l) {
        function m(a, b) {
            var c, d, e = new Image();
        ...
        var p = new AWS.S3(), q = this;
        q.params = b, q.order = d.payload.data;
        var r = function() {
        ...
        q.goBack = function() {
        ...
        }, q.downloadPop = function() {
            var a = {
                Bucket: j.S3_BUCKET,
                Prefix: q.params.id + "/" + q.order.pickedup_datetime.slice(0, 10) + "/pop"
            };
            q.download_image(a);
        }, q.downloadPod = function() {
            var a = {
                Bucket: j.S3_BUCKET,
                Prefix: q.params.id + "/" + q.order.pickedup_datetime.slice(0, 10) + "/pod"
            };
            q.download_image(a);
        }, q.download_image = function(a) {
            l.loaderStart(), p.listObjects(a, function(a, b) {
                if (a) l.loaderComplete(), l.showError(a); else {
                    if (0 === b.Contents.length) return l.loaderComplete(), void l.showError("No Proof Found");
                    if (r()) {
                        ...
                        return l.loaderComplete(), void l.showError(c);
                    }
                    async.map(b.Contents, o, function(a, b) {
                        a ? (l.loaderComplete(), l.showError(a)) : (l.loaderComplete(), l.showSuccess("Proof Download Successful"));
                    });
                }
            });
            ...
            l.loaderStart(), k.all(b).then(function(a) {
                q.getOrder();
            });
        ...
        }, q.statusUpdateDialog = function(a) {
            h.openStatusDialog().then(function(b) {
                b.delivery_ids = [ a.id ], b.isPickup ? q.updatePickupStatus(b) : q.updateDeliveryStatus(b);
            });
        }, q.getOrder = function() {
    ...
    angular.module("order").controller("orderDetailCntrl", [ "$state", "$stateParams", "$rootScope", "order", "DeliveryGuy", "Order", "orderDgAssign", "OrderStatusUpdate", "PreviousState", "constants", "$q", "Notification", a ]);
}(), function() {
    "use strict";
    ...
    var a = function(a, b, c, d, e) {
        var f = this;
        f.params = b, f.searchVendor = f.params.vendor_id, f.report_stats = d.payload.data, 
        console.log(f.report_stats), f.vendorSearchTextChange = function(a) {
            console.log(a), a || console.log("none");
            var b = {
                search: a
            };
            ...
            a ? f.params.vendor_id = a.id : f.params.vendor_id = void 0, f.getReports();
        }, f.manipulateGraph = function() {
            f.graphData = {
                chart: {
                    plotGradientColor: " ",
                    plotSpacePercent: "60",
            ...
            for (var a = 0; a < f.report_stats.orders.length; a++) f.graphData.categories[0].category[a] = {}, 
            f.graphData.categories[0].category[a].label = f.report_stats.orders[a].date.slice(8, 10), 
            f.graphData.dataset[0].data[a] = {
                value: f.report_stats.orders[a].delivered_count,
                toolText: "Total Delivered:" + f.report_stats.orders[a].delivered_count + "{br} Total Placed:" + f.report_stats.orders[a].total_orders_count + "{br}" + new Date(f.report_stats.orders[a].date).toDateString()
            }, f.graphData.dataset[1].data[a] = {
                value: f.report_stats.orders[a].delivery_attempted_count + f.report_stats.orders[a].pickup_attempted_count,
                toolText: "Total Attempted:" + (f.report_stats.orders[a].delivery_attempted_count + f.report_stats.orders[a].pickup_attempted_count) + "{br} Total Placed:" + f.report_stats.orders[a].total_orders_count + "{br}" + new Date(f.report_stats.orders[a].date).toDateString()
            }, f.graphData.dataset[2].data[a] = {
                value: f.report_stats.orders[a].intransit_count,
                toolText: "Total Intransit:" + f.report_stats.orders[a].intransit_count + "{br} Total Placed:" + f.report_stats.orders[a].total_orders_count + "{br}" + new Date(f.report_stats.orders[a].date).toDateString()
            }, f.graphData.dataset[3].data[a] = {
                value: f.report_stats.orders[a].queued_count,
                toolText: "Total Queued:" + f.report_stats.orders[a].queued_count + "{br} Total Placed:" + f.report_stats.orders[a].total_orders_count + "{br}" + new Date(f.report_stats.orders[a].date).toDateString()
            }, f.graphData.dataset[4].data[a] = {
                value: f.report_stats.orders[a].cancelled_count,
                toolText: "Total Cancelled:" + f.report_stats.orders[a].cancelled_count + "{br} Total Placed:" + f.report_stats.orders[a].total_orders_count + "{br}" + new Date(f.report_stats.orders[a].date).toDateString()
            };
            console.log(f.graphData);
        }, 0 !== f.report_stats.total_orders && f.manipulateGraph(), f.getReports = function() {
            e.loaderStart(), a.transitionTo(a.current, f.params, {
                reload: !0,
                inherit: !1,
                notify: !0
            });
        ...
        a.state("home.reports", {
            ...
            resolve: {
                ...
                report: [ "Reports", "$stateParams", function(a, b) {
                    return b.start_date = void 0 !== b.start_date ? moment(b.start_date).toISOString() : moment().set("date", 1).toISOString(), 
                    b.end_date = void 0 !== b.end_date ? moment(b.end_date).toISOString() : moment().toISOString(), 
                    a.getReport.stats(b).$promise;
                } ]
            }
        });
    } ]).controller("reportsCntrl", [ "$state", "$stateParams", "Vendor", "report", "Notification", a ]);
}();
