/*! Vendor-Webaap - v0.0.0 - 2016-01-21 */
...
this.handleOrdeSelection={selectActive:i.isSelected(),numberOfSelectedOrder:i.slectedItemLength(),update:function(){j.handleOrdeSelection.selectActive=i.isSelected(),j.handleOrdeSelection.numberOfSelectedOrder=i.slectedItemLength()},toggle:function(a){i.toggle(a),j.handleOrdeSelection.update()},exists:function(a){return i.exists(a)},clear:function(){i.clearAll(),j.handleOrdeSelection.update()}},/*
			@getOrders rleoads the order controller according too the filter to get the new filtered data.
		*/
this.getOrders=function(){a.transitionTo(a.current,j.params,{reload:!0,inherit:!1,notify:!0})}},b=function(a){console.log("vendor")};angular.module("order",[]).config(["$stateProvider",function(a){a.state("home.opsorder",{url:"^/all-orders?date&vendor&dg&status&page",templateUrl:"/static/modules/order/opsOrders.html",controllerAs:"opsOrder",controller:"opsOrderCntrl",resolve:{vendorClients:"vendorClients",orderResource:"Order",DeliverGuy:"DeliverGuy",access:["Access","constants",function(a,b){return a.hasRole(b.userRole.ADMIN)}],orders:["Order","$stateParams",function(a,b){return b.date=void 0!==b.date?new Date(b.date).toISOString():(new Date).toISOString(),b.page=isNaN(b.page)?1:parseInt(b.page),a.getOrders.query(b).$promise}]}}).state("home.order",{url:"^/orders",templateUrl:"/static/modules/order/vendorOrders.html",controllerAs:"order",controller:"vendorOrderCntrl",resolve:{vendorClients:"vendorClients",access:["Access","constants",function(a,b){return a.hasRole(b.userRole.VENDOR)}]}})}]).controller("opsOrderCntrl",["$state","$mdSidenav","$stateParams","vendorClients","orderResource","orders","DeliverGuy","constants","orderSelection",a]).controller("vendorOrderCntrl",["$state",b])}(),function(){"use strict";var a=function(){return a};angular.module("order").factory("orderFilter",[a])}(),function(){"use strict";var a=function(){var a={selectedItemArray:[],toggle:function(b){var c=a.selectedItemArray.indexOf(b);c>-1?a.selectedItemArray.splice(c,1):a.selectedItemArray.push(b)},exists:function(b){return a.selectedItemArray.indexOf(b)>-1},addItem:function(b){b.selected=!0,a.selectedItemArray.push(b)},removeItem:function(b){var c=selectedItemArray.indexOf(b);return c>-1?(b.selected=!1,a.selectedItemArray.splice(c,1),!0):!1},isSelected:function(){return a.selectedItemArray.length>0?!0:!1},clearAll:function(){return a.selectedItemArray=[],a.selectedItemArray},slectedItemLength:function(){return a.selectedItemArray.length}};return a};angular.module("order").factory("orderSelection",[a])}(),function(){"use strict";angular.module("deliveryguy",[]).config(["$stateProvider",function(a){a.state("home.dgList",{url:"^/deliveryguy/list?date&search&page",templateUrl:"/static/modules/deliveryguy/list/list.html",controllerAs:"dgList",controller:"dgListCntrl",resolve:{dgs:["DeliverGuy","$stateParams",function(a,b){return b.date=void 0!==b.date?new Date(b.date).toISOString():(new Date).toISOString(),b.page=isNaN(b.page)?1:parseInt(b.page),a.dgPageQuery.query(b).$promise}]}})}])}(),function(){"use strict";/*
		dgListCntrl is the controller for the delivery guy lst page. 
		Its resolved after loading all the dgs from the server.
			
	*/
var a=function(a,b,c,d){var e=this;this.params=c,/*
			@dgs: resolved dgs list accordign to the url prameters.
		*/
this.dgs=d.data,this.total_pages=d.total_pages,this.total_dgs=d.total_dg_count,/*
			 @ toggleFilter : main sidenav toggle function, this function toggle the sidebar of the filets of the orders page page.
		*/
this.toggleFilter=function(){b("dgList-filter").toggle()},/*
			...
			@statusSelection is a fucntion to select or unselect the status data in order filter
		*/
this.paginate={nextpage:function(){e.params.page=e.params.page+1,e.getDgs()},previouspage:function(){e.params.page=e.params.page-1,e.getDgs()}},/*
			@getOrders rleoads the order controller according too the filter to get the new filtered data.
		*/
this.getDgs=function(){a.transitionTo(a.current,e.params,{reload:!0,inherit:!1,notify:!0})}};angular.module("deliveryguy").controller("dgListCntrl",["$state","$mdSidenav","$stateParams","dgs",a])}();
